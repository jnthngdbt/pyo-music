...
## -------------------------------------------------------
Tw = 12 # sample duration
winRatio = 1.0
crossFadeRatio = 0.4 #1.5 * winRatio
lowPass = 2000
...
songLen = x.shape[0]
nbSegSamples = Tw * fs
maxSegPosIdx = songLen - nbSegSamples - 1
winLen = int(winRatio * nbSegSamples)
crossFadeLen = int(crossFadeRatio * nbSegSamples)
...
s = []
si = np.zeros((nbSegSamples, x.shape[1]))
for i in np.arange(nbSegments):
  print('{0}/{1}'.format(i, nbSegments))
  posStart = int(maxSegPosIdx * np.random.rand())
  posEnd = posStart + nbSegSamples
  si[:,:] = x[posStart:posEnd, :]
  si = applyWindow(si, signal.windows.hann(winLen))
  s, ti = mixSignal(s, si, crossFadeLen)

pathOut = "./songs/"
nameOut = '{}{}.{}segments.Tw{}ms.LP{}Hz.win{}.crossfade{}.m4a'.format(pathOut, name, int(nbSegments), int(Tw*1000), int(lowPass), int(winRatio*100), int(crossFadeRatio*100))
