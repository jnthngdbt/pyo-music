...
def processSample(x, k):
  ...
  x = applyWindow(x, signal.windows.hann(int(0.9 * x.shape[0])))
...
Ns = 44000
Nw = 11000
Tk = 10 # desired final sample duration, should be longer than sample duration

Nk = Tk * fs
k = int(Nk / Nw)

s = []
for i in np.arange(x.shape[0] - Nw - 1, step=Ns):
  s = mixSignal(s, processSample(x[i:i+Nw, :], k), int(0.5 * Nk))
